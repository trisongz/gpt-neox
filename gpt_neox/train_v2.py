...
from gpt_neox.arguments import get_args
from gpt_neox.configure_data import configure_data
from gpt_neox.fp16 import FP16_Module
from gpt_neox.fp16 import FP16_Optimizer
from gpt_neox.learning_rates import AnnealingLR
from gpt_neox.model import GPT2Model
from gpt_neox.model import gpt2_get_params_for_weight_decay_optimization

if USE_TORCH_DDP:
    from torch.nn.parallel.distributed import DistributedDataParallel as DDP
else:
    from gpt_neox.model import DistributedDataParallel as DDP
from gpt_neox import mpu
from apex.optimizers import FusedAdam as Adam
from gpt_neox.ds_utils import Timers
from gpt_neox.ds_utils import save_checkpoint
from gpt_neox.ds_utils import load_checkpoint
from gpt_neox.ds_utils import report_memory
from gpt_neox.ds_utils import print_args
from gpt_neox.ds_utils import print_params_min_max_norm
from gpt_neox.ds_utils import print_rank_0
import torch.distributed as dist

from gpt_neox.gpt2_data_loader import make_gpt2_dataloaders
...
def get_model(args):
    ...
    if USE_TORCH_DDP:
        ...
        model = DDP(model, device_ids=[i], output_device=i, process_group=mpu.get_data_parallel_group())
    else:
        model = DDP(model)
...
def train(model, optimizer, lr_scheduler, train_data_iterator, val_data_iterator, timers, args):
    """Train the model."""
    ...
    while iteration < args.train_iters:
        ...
        if args.exit_interval and iteration % args.exit_interval == 0:
            ...
            print('rank: {} | time: {} | exiting the program at iteration {}'.format(rank, time_str, iteration), flush=True)
            exit()
...
def evaluate_and_print_results(prefix, data_iterator, model, args, timers, verbose=False):
...
def main():
    ...
    if args.train_iters > 0:
        if args.do_train:
            iteration, skipped = train(model, optimizer, lr_scheduler, train_data_iterator, val_data_iterator, timers, args)

        if args.do_valid:
            prefix = 'the end of training for val data'
            val_loss = evaluate_and_print_results(prefix, val_data_iterator, model, args, timers, False)
    ...
    if args.do_test:
        ...
        evaluate_and_print_results(prefix, test_data_iterator, model, args, timers, True)
